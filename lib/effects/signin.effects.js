...
var SigninEffects = /** @class */ (function () {
    function SigninEffects(actions$, router, signinService) {
        ...
        this.preSignUpStart$ = this.actions$
            ...
            .switchMap(function (payload) { return _this.signinService.signin(payload); })
            .map(function (user) { return new signin_actions_1.SigninSecceed(user); })
            .catch(function (error) { return Observable_1.Observable.of(new signin_actions_1.SigninFailed(error)); });
        this.SigninSucceed$ = this.actions$
            .ofType(signin_actions_1.SignInActionTypes.SIGNIN_SUCCEED)
            .map(effects_1.toPayload)
            .do(function (data) { return _this.router.navigate(["/"]); });
        ...
        this.redirectToLoginPage$ = this.actions$
            ...
            .do(function (authed) { return _this.router.navigate(["auth/signin"]); });
    }
